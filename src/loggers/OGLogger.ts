...
/**
 * This is the original logger class that I exported from the library. I want to redo this class so
 * it has support for transports and a variety of other features so I'm keeping this as a record.
 */
export class OGLogger {
  ...
  /**
   * The sole constructor available for the `Logger` class. It takes an optional `ILoggerOpts`
   * object which can be used to modify the output of the logger. If no options are provided, the
   * default options are used.
   *
   * @param opts An optional object containing the options for the logger.
   */
  public constructor(opts?: ILoggerOptions) {
    let colorOpts = DEFAULT_COLOR_OPTIONS
    let formatOpt = DEFAULT_LOG_FORMAT

    //@ts-ignore This isn't actually being used/transpiled.
    if (opts?.outputFormat) formatOpt = opts.outputFormat
